...
if __name__ == "__main__":
    ...
    flag = True
    with open(os.path.join(data_path, "results/feats_global.csv"), "w") as f:
        w = csv.writer(f)
    ...
    df = pd.read_csv(os.path.join(data_path, "results/feats_global.csv"), index_col="subject")
    normalization_df = (df - df.mean()) / df.std()
    normalization_df.to_csv(os.path.join(data_path, "results/nfeats_global.csv"))